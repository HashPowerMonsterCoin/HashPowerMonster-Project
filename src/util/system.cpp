...
const char * const BITCOIN_CONF_FILENAME = "hashpowermonster.conf";
...
#else
    const char* pszModule = "hashpowermonster";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HashpowerMonster";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HashpowerMonster";
#else
    // Unix
    return pathRet / ".hashpowermonster";
#endif
#endif
}
